...
internal class CinematicFrameRenderer(timelineFile: File) {
    ...
    private val highlightStrokePaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        color = Color.argb(125, 145, 145, 157)
    ...
    init {
        ...
        for (i in 0 until rawParagraphs.length()) {
            ...
            val layout = StaticLayout.Builder
                .obtain(text, 0, text.length, paragraphPaint, (CARD_W - 2f * PAD_X).toInt())
    ...
    private fun findTextSize(text: String): Float {
        for (size in 37 downTo 31) {
            ...
            val layout = StaticLayout.Builder
                .obtain(text, 0, text.length, paint, (CARD_W - 2f * PAD_X).toInt())
                .setAlignment(Layout.Alignment.ALIGN_CENTER)
                .setIncludePad(false)
                .setLineSpacing(0f, 1.72f)
                .build()
    ...
    private fun makeStars(
        ...
        return List(count) {
            ...
            Star(
                ...
                vx = (-maxVx + random.nextFloat() * 2f * maxVx) * d,
                vy = (-maxVy + random.nextFloat() * 2f * maxVy) * d,
                radius = (minR + random.nextFloat() * (maxR - minR)) * d / maxD,
                alpha = minA + random.nextFloat() * (maxA - minA),
    ...
    private fun makeFastStars(): List<Star> {
        ...
        return List(7) {
            ...
            Star(
                ...
                vx = cos(a) * speed,
                vy = sin(a) * speed,
                radius = 12f + random.nextFloat() * 26f,
                alpha = 65f + random.nextFloat() * 60f,
                phase = random.nextFloat() * 25f,
            )
        }
    }
    ...
    private fun drawBackground(t: Double) {
        ...
        for (star in slowStars) {
            ...
            val a = (star.alpha * (0.82 + 0.18 * sin(t * 0.45 + star.phase))).toInt().coerceIn(0, 255)
            starPaint.color = Color.argb(a, 205, 215, 245)
            canvas.drawCircle(x, y, star.radius, starPaint)
        }
        for (star in mediumStars) {
            val x = mod(star.x + star.vx * t.toFloat(), WIDTH.toFloat())
            val y = mod(star.y + star.vy * t.toFloat(), HEIGHT.toFloat())
            val a = (star.alpha * (0.76 + 0.24 * sin(t * 0.8 + star.phase))).toInt().coerceIn(0, 255)
            starPaint.color = Color.argb(a, 222, 230, 255)
            canvas.drawCircle(x, y, star.radius, starPaint)
        }
        ...
        shapePaint.shader = RadialGradient(
            WIDTH / 2f, HEIGHT / 2f, 930f,
            intArrayOf(Color.TRANSPARENT, Color.argb(110, 0, 0, 0)),
            floatArrayOf(0.63f, 1f), Shader.TileMode.CLAMP
        )
        canvas.drawRect(0f, 0f, WIDTH.toFloat(), HEIGHT.toFloat(), shapePaint)
        shapePaint.shader = null
    }
    ...
    private fun drawNebula(cx: Float, cy: Float, rx: Float, ry: Float, color: Int) {
        shapePaint.shader = RadialGradient(
            cx, cy, max(rx, ry),
            intArrayOf(Color.argb(92, Color.red(color), Color.green(color), Color.blue(color)), Color.TRANSPARENT),
    ...
    private fun drawCard(p: Paragraph, tMs: Int) {
        ...
        canvas.drawRoundRect(RectF(rect.left + 10f, rect.top + 10f, rect.right - 10f, rect.bottom - 10f), 22f, 22f, shapePaint)
        shapePaint.style = Paint.Style.FILL
        ...
        if (active != null) {
            ...
            if (end > start) {
                ...
                canvas.save()
                canvas.translate(CARD_X, textTop)
                canvas.drawPath(selectionPath, highlightPaint)
                canvas.restore()
            }
        }
        canvas.save()
