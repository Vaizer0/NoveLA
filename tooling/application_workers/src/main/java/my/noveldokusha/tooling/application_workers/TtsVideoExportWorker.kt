...
class TtsVideoExportWorker(context: Context, params: WorkerParameters) : CoroutineWorker(context, params) {
    ...
    override suspend fun doWork(): Result {
        ...
        try {
            setForeground(ForegroundInfo(NOTIFICATION_ID, notification(request.chapterTitle, 1), if (Build.VERSION.SDK_INT >= 29) ServiceInfo.FOREGROUND_SERVICE_TYPE_DATA_SYNC else 0))
            ...
            val synthesis = TtsVideoAudioSynthesizer(applicationContext).synthesize(request, textBlocks, wav) { f ->
                val p = (f * 45f).toInt().coerceIn(1,45); update(prefs,request,TtsVideoJobStatus.RUNNING,p); setProgressAsync(Data.Builder().putInt("progress",p).build())
            }
            ...
            TtsVideoMp4Encoder().encode(wav,mp4,timeline,request.visual,renderer,snapshot) { f ->
                val p=(50+f*45f).toInt().coerceIn(50,95);update(prefs,request,TtsVideoJobStatus.RUNNING,p);setProgressAsync(Data.Builder().putInt("progress",p).build())
            }
            val outUri=withContext(Dispatchers.IO){publish(applicationContext,Uri.parse(request.outputDirectoryUri),request,mp4)} ?: return fail(prefs,request,"Unable to create video file")
            temp.deleteRecursively(); update(prefs,request,TtsVideoJobStatus.SUCCESS,100,outUri.toString()); return Result.success()
        } catch(e:CancellationException){update(prefs,request,TtsVideoJobStatus.CANCELLED,0,message="cancelled");return Result.failure()}
        catch(e:Throwable){update(prefs,request,TtsVideoJobStatus.FAILED,0,message=e.message ?: "Video export failed");return Result.failure()}
    }
    ...
    private suspend fun loadText(db: AppDatabase, request: TtsVideoRequest): List<String>? = if (request.source == TtsAudioSource.ORIGINAL) {
        ...
        runCatching { val a=JSONArray(row.translatedParagraphs); (0 until a.length()).mapNotNull{a.optString(it).takeIf(String::isNotBlank)} }.getOrNull()
    }
    ...
    private fun update(p:TtsVideoPreferences,r:TtsVideoRequest,status:TtsVideoJobStatus,progress:Int,output:String="",message:String=""){
        val jobs=p.jobs().toMutableMap();val old=jobs[r.jobId]?:TtsVideoJobState(r.chapterUrl,r.novelUrl,r.chapterTitle,r.source,status);jobs[r.jobId]=old.copy(status=status,progress=progress,outputUri=output.ifBlank{old.outputUri},message=message);p.saveJobs(jobs)
    }
    ...
    private fun notification(title:String,progress:Int):Notification{val m=applicationContext.getSystemService(NotificationManager::class.java);if(Build.VERSION.SDK_INT>=26)m.createNotificationChannel(NotificationChannel(CHANNEL,"TTS Video",NotificationManager.IMPORTANCE_LOW));return NotificationCompat.Builder(applicationContext,CHANNEL).setSmallIcon(android.R.drawable.stat_sys_upload).setContentTitle("Creating video").setContentText(title).setProgress(100,progress,false).setOngoing(true).build()}
