...
/**
 * Renders already-composed bitmaps into a MediaCodec input Surface and assigns an
 * explicit presentation timestamp to every submitted frame. No external native
 * multimedia library is required.
 */
internal class TtsVideoEglSurfaceRenderer(private val surface: Surface) : AutoCloseable {
    ...
    fun draw(bitmap: Bitmap, presentationTimeUs: Long) {
        ...
        GLES20.glViewport(0, 0, bitmap.width.coerceAtMost(1920), bitmap.height.coerceAtMost(1080))
        ...
        check(EGLExt.eglPresentationTimeANDROID(display, eglSurface, presentationTimeUs.coerceAtLeast(0L) * 1_000L)) {
            "Unable to set EGL presentation timestamp"
        }
    ...
    private fun initialize() {
        ...
        check(EGL14.eglChooseConfig(display, configAttrs, 0, configs, 0, 1, count, 0) && count[0] == 1) {
    ...
    private companion object {
        // Two triangles, bottom-left first so the Android Bitmap is not vertically flipped.
        val FULLSCREEN_VERTICES = java.nio.ByteBuffer.allocateDirect(4 * 2 * 4)
            .order(java.nio.ByteOrder.nativeOrder()).asFloatBuffer().apply {
                put(floatArrayOf(-1f, -1f, 1f, -1f, -1f, 1f, 1f, 1f)); position(0)
            }
        val TEX_COORDS = java.nio.ByteBuffer.allocateDirect(4 * 2 * 4)
            .order(java.nio.ByteOrder.nativeOrder()).asFloatBuffer().apply {
