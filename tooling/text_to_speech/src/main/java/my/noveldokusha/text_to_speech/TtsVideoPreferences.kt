...
class TtsVideoPreferences(context: Context) {
    ...
    fun visualSettings(): TtsVideoVisualSettings = TtsVideoVisualSettings(
        ...
        paragraphSpacingPx = prefs.getFloat("paragraphSpacingPx", 28f),
        letterSpacingEm = prefs.getFloat("letterSpacingEm", 0f),
        ...
        cardStrokeWidthPx = prefs.getFloat("cardStrokeWidthPx", 1.5f),
        cardStrokeColor = prefs.getInt("cardStrokeColor", android.graphics.Color.argb(80, 255, 255, 255)),
        highlightColor = prefs.getInt("highlightColor", android.graphics.Color.YELLOW),
        highlightAlpha = prefs.getFloat("highlightAlpha", .92f),
        highlightCornerRadiusPx = prefs.getFloat("highlightCornerRadiusPx", 8f),
        highlightPaddingPx = prefs.getFloat("highlightPaddingPx", 4f),
        safeMarginPx = prefs.getFloat("safeMarginPx", 80f),
        maxTextWidthFraction = prefs.getFloat("maxTextWidthFraction", .82f),
        ...
        artworkUris = stringList("artworkUris"),
        artworkWidthPx = prefs.getFloat("artworkWidthPx", 260f),
        artworkOpacity = prefs.getFloat("artworkOpacity", .95f),
        artworkCornerRadiusPx = prefs.getFloat("artworkCornerRadiusPx", 24f),
        artworkBorderWidthPx = prefs.getFloat("artworkBorderWidthPx", 0f),
        artworkBorderColor = prefs.getInt("artworkBorderColor", android.graphics.Color.TRANSPARENT),
        artworkOverlay = prefs.getBoolean("artworkOverlay", false),
    ...
    fun putVisual(settings: TtsVideoVisualSettings) {
        prefs.edit()
            ...
            .putFloat("fontSizePx", settings.fontSizePx)
            .putFloat("minFontSizePx", settings.minFontSizePx)
            .putFloat("lineSpacingMultiplier", settings.lineSpacingMultiplier)
            .putFloat("paragraphSpacingPx", settings.paragraphSpacingPx)
            .putFloat("letterSpacingEm", settings.letterSpacingEm)
            .putInt("textColor", settings.textColor)
            .putBoolean("cardEnabled", settings.cardEnabled)
            .putInt("cardColor", settings.cardColor)
            .putFloat("cardAlpha", settings.cardAlpha)
            .putFloat("cardCornerRadiusPx", settings.cardCornerRadiusPx)
            .putFloat("cardPaddingPx", settings.cardPaddingPx)
            .putFloat("cardStrokeWidthPx", settings.cardStrokeWidthPx)
            .putInt("cardStrokeColor", settings.cardStrokeColor)
            .putInt("highlightColor", settings.highlightColor)
            .putFloat("highlightAlpha", settings.highlightAlpha)
            .putFloat("highlightCornerRadiusPx", settings.highlightCornerRadiusPx)
            .putFloat("highlightPaddingPx", settings.highlightPaddingPx)
            .putFloat("safeMarginPx", settings.safeMarginPx)
            .putFloat("maxTextWidthFraction", settings.maxTextWidthFraction)
            .putString("backgroundMode", settings.backgroundMode.name)
            .putInt("backgroundColor", settings.backgroundColor)
            .putString("backgroundUri", settings.backgroundUri)
            .putString("artworkMode", settings.artworkMode.name)
            .putString("artworkUris", JSONArray(settings.artworkUris).toString())
            .putFloat("artworkWidthPx", settings.artworkWidthPx)
            .putFloat("artworkOpacity", settings.artworkOpacity)
            .putFloat("artworkCornerRadiusPx", settings.artworkCornerRadiusPx)
            .putFloat("artworkBorderWidthPx", settings.artworkBorderWidthPx)
            .putInt("artworkBorderColor", settings.artworkBorderColor)
            .putBoolean("artworkOverlay", settings.artworkOverlay)
            .putBoolean("slideshowEnabled", settings.slideshowEnabled)
            .putString("slideshowIntervalMode", settings.slideshowIntervalMode.name)
            .putLong("slideshowIntervalMs", settings.slideshowIntervalMs)
            .putString("slideshowTransition", settings.slideshowTransition.name)
            .putLong("slideshowSeed", settings.slideshowSeed)
            .apply()
    }

    fun jobs(): Map<String, TtsVideoJobState> = runCatching {
        val array = JSONArray(prefs.getString(KEY_JOBS, "[]") ?: "[]")
        buildMap<String, TtsVideoJobState> {
            for (i in 0 until array.length()) {
                val o = array.getJSONObject(i)
                val id = o.getString("id")
                put(id, TtsVideoJobState(
                    chapterUrl = o.getString("chapterUrl"),
                    novelUrl = o.getString("novelUrl"),
                    chapterTitle = o.getString("chapterTitle"),
                    source = TtsAudioSource.valueOf(o.getString("source")),
                    status = TtsVideoJobStatus.valueOf(o.getString("status")),
                    workRequestId = o.optString("workRequestId"),
    ...
    fun saveJobs(jobs: Map<String, TtsVideoJobState>) {
        ...
        jobs.forEach { (id, job) ->
            array.put(JSONObject().apply {
                put("id", id)
                put("chapterUrl", job.chapterUrl)
                put("novelUrl", job.novelUrl)
                put("chapterTitle", job.chapterTitle)
                put("source", job.source.name)
                put("status", job.status.name)
                put("workRequestId", job.workRequestId)
                put("progress", job.progress)
                put("outputUri", job.outputUri)
