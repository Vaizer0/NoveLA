...
class TtsVideoPreferences(context: Context) {
    ...
    fun visualSettings(): TtsVideoVisualSettings = TtsVideoVisualSettings(
        ...
        lineSpacingMultiplier = prefs.getFloat("lineSpacingMultiplier", 1.15f), textColor = prefs.getInt("textColor", android.graphics.Color.WHITE),
        cardEnabled = prefs.getBoolean("cardEnabled", true), cardColor = prefs.getInt("cardColor", android.graphics.Color.DKGRAY), cardAlpha = prefs.getFloat("cardAlpha", .82f),
        cardCornerRadiusPx = prefs.getFloat("cardCornerRadiusPx", 28f), cardPaddingPx = prefs.getFloat("cardPaddingPx", 30f),
        ...
        backgroundMode = enum("backgroundMode", BackgroundMode.SOLID), backgroundColor = prefs.getInt("backgroundColor", android.graphics.Color.rgb(18,18,22)), backgroundUri = prefs.getString("backgroundUri", "") ?: "",
        artworkMode = enum("artworkMode", ArtworkMode.NONE), artworkUris = stringList("artworkUris"), artworkWidthPx = prefs.getFloat("artworkWidthPx", 260f), artworkOpacity = prefs.getFloat("artworkOpacity", .95f),
        ...
        slideshowIntervalMs = prefs.getLong("slideshowIntervalMs", 8000L), slideshowTransition = enum("slideshowTransition", SlideshowTransition.CROSSFADE), slideshowSeed = prefs.getLong("slideshowSeed", 0L),
    )
    ...
    fun putVisual(settings: TtsVideoVisualSettings) {
        prefs.edit()
            ...
            .putString("artworkMode", settings.artworkMode.name).putString("artworkUris", JSONArray(settings.artworkUris).toString()).putFloat("artworkWidthPx", settings.artworkWidthPx).putFloat("artworkOpacity", settings.artworkOpacity)
        ...
        buildMap<String, TtsVideoJobState> {
            for (i in 0 until array.length()) {
                ...
                put(id, TtsVideoJobState(
                    ...
                    workRequestId = o.optString("workRequestId"), progress = o.optInt("progress"), outputUri = o.optString("outputUri"), message = o.optString("message")
                ))
            }
        }
    ...
    fun saveJobs(jobs: Map<String, TtsVideoJobState>) {
        ...
        jobs.forEach { (id, job) -> array.put(JSONObject().apply {
            ...
            put("source", job.source.name); put("status", job.status.name); put("workRequestId", job.workRequestId); put("progress", job.progress); put("outputUri", job.outputUri); put("message", job.message)
        }) }
    ...
    companion object { private const val PREFS_NAME = "novela_tts_video"; const val KEY_OUTPUT_URI = "TTS_VIDEO_DOWNLOAD_LOCATION_URI"; const val KEY_JOBS = "TTS_VIDEO_DOWNLOAD_JOBS" }
}
